...
from models.DiMeR.models.geometry.render import renderutils as ru
import numpy as np
...
def shade_with_env(gb_pos, gb_normal, kd, metallic, roughness, view_pos, run_n_view, env, metallic_gt, roughness_gt, use_material_gt=True, gt_render=False):
    ...
    for i in range(run_n_view):
        ...
        if use_material_gt:
            ...
            fg_uv = torch.cat((NdotV, roughness[i,:,:,:]), dim=-1)
        #if not hasattr(self, '_FG_LUT'):
        _FG_LUT = torch.as_tensor(np.fromfile('src/data/irrmaps/bsdf_256_256.bin', dtype=np.float32).reshape(1, 256, 256, 2), dtype=torch.float32, device='cuda')
...
def shade(
    ...
    if not use_PBR:
        # if not use PBR, we the output of kd can be directly used as color
        ...
        spec_light = torch.zeros_like(kd)
        diff_light = torch.zeros_like(kd)
        ...
        buffers = {
            'shaded': torch.cat((kd, alpha), dim=-1),
    ...
    if planes is None:
        ...
        if gt_albedo_map is not None:
            ...
            kd, matellic, roughness = kd_fn( planes[None,...], gb_pos_interp, mask[0])
            kd = torch.cat( [torch.cat([kd[i:i + 1, :, resolution * i_view: resolution * (i_view + 1)]for i_view in range(N_views)], dim=0) for i in range(len(kd))], dim=0)
            matellic = torch.cat( [torch.cat([matellic[i:i + 1, :, resolution * i_view: resolution * (i_view + 1)]for i_view in range(N_views)], dim=0) for i in range(len(matellic))], dim=0)
            roughness = torch.cat( [torch.cat([roughness[i:i + 1, :, resolution * i_view: resolution * (i_view + 1)]for i_view in range(N_views)], dim=0) for i in range(len(roughness))], dim=0)
            use_material_gt = False
    ...
    shaded_col, spec_light, diff_light = shade_with_env(gb_pos, gb_normal_, kd, matellic, roughness, view_pos, N_views, env, matellic_gt, roughness_gt, use_material_gt=use_material_gt, gt_render=gt_render)
...
def render_layer(
    ...
    # Shade
    ################################################################################
    buffers = shade(gb_pos, gb_geometric_normal, gb_normal, gb_tangent, gb_texc, gb_texc_deriv, view_pos, env, planes, kd_fn, materials, mesh.material, mask, gt_render, gt_albedo_map=gt_albedo_map, use_PBR=use_PBR)
...
def render_mesh(
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), full_res) as peeler:
        for _ in range(num_layers):
            ...
            layers += [(render_layer(rast, db, mesh, view_pos, env, planes, kd_fn, materials, v_pos_clip, resolution, spp, msaa, gt_render, gt_albedo_map, use_PBR), rast)]
